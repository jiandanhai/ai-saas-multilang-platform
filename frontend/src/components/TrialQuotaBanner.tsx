...
const TrialQuotaBanner: React.FC<TrialQuotaBannerProps> = ({ quotaLeft }) => {
  ...
  // Adjust colors and tips based on remaining quota
  ...
  return (
    <div className={`mt-16 mb-6 px-0 w-full flex justify-center`}>
      <div
        className={`bg-gradient-to-r ${statusColor} rounded-xl shadow-lg px-5 py-2 flex items-center gap-3 transition-all duration-500 relative animate-fadein`}
      ...
      >
        ...
        <span className="relative">
          ...
          {showInfo && (
            <span className="absolute left-1/2 -translate-x-1/2 mt-3 z-50 text-xs bg-white text-gray-700 shadow-lg rounded-xl px-4 py-2 border border-gray-200 whitespace-nowrap animate-popin">
              {quotaTip}
              <div className="pt-2 text-[11px] text-gray-400">注册/升级套餐可解锁不限额度</div>
            </span>
          )}
        </span>
      </div>
